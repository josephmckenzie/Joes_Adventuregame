...
def wanna_play
	...
	if gamechoice =~ /[[:digit:]]/  || gamechoice !~ /[[:alpha:]]/   
		puts "Sorry that was an invalid selection. Please Choose again" 
		wanna_play
	elsif	gamechoice.include? ("Y") or gamechoice.include?("S") 
		puts "\n\n#{@user_name} is sitting at home when the phone rings...... \nDo you answer it?"
		incoming_call
		answer_phone_?
	else gamechoice.include? ("N") 
		abort("Why would you even start a game and then not want to play it?")
	end
end
...
def answer_phone_?
	...
	if phonechoice1 =~ /[[:digit:]]/  || phonechoice1 !~ /[[:alpha:]]/   
		puts "Sorry that was an invalid selection. Please Choose again" 
		answer_phone_?
	elsif phonechoice1.include? ("Y") or phonechoice1.include?("S")
		answer_phone_call
	else phonechoice1.include? ("N")
		puts """Common #{@user_name} Answer the phone man dont miss out. Will you answer ?"""
	if phonechoice1 =~ /[[:digit:]]/  || phonechoice1 !~ /[[:alpha:]]/   
		puts "Sorry that was an invalid selection. Please Choose again" 
		answer_phone_?
	print "\n> "
	...
	elsif phonechoice2.include? ("Y") or phonechoice2.include?("S")
		answer_phone_call
...
planechoice= $stdin.gets.chomp.upcase
	if planechoice.include? ("AM")
		...
		cockpit
		"\nWelcome to Africa, The weather today is 120 degrees... You arrive at the airport in Africa and are met by a Big ol Tourguide"
		entertruck
	else planechoice.include? ("DE")
		puts "Thank you For flying with Delta today, you had your choice of airlines and well you chose the wrong one.......
		Our flight time should be around 7 hours barring any unforeseen circumstances such as pilot error or just plane not caring....(get it.....Plane?) 
...
def delta_bathroom
    ...
    if dechoice1.include?("Y") or dechoice1.include?("S")
		puts """While getting up to use the restroom, The flight attendants tell you to sit your butt back down.... 
		When you ask why? They say because We said so and up here we are the boss...... \nDo you sit back down?"""	
    print ">"
    dechoice2=$stdin.gets.chomp.upcase
		if dechoice2.include?("Y") or dechoice2.include?("S")
		puts "man they suck"
    elsif dechoice2.include? ("N")
		puts  "You go to the restroom and after using the restroom you go back to your seat. 
		Lucky that they dont sic the air marshal on you but they don't."
        cockpit
		print ">"
        entertruck
        africa
      elsif dechoice1.include? ("N")
		puts "You decide to wait to use the restroom until you land because you are not going 	in that restroom after THAT dude just did...."
	end
  end
end
...
def africa
	@user_name
	truck
	puts """#{@user_name} and their new tour guide set off to the campsite which is around 3 hours away.... 
